...
<!-- TABLE START -->
<div class="row">
    <div class="col-lg-12" id="card-none1">
        <div class="card">
            <div class="card-header pt-2 pb-2">
                <div class="d-flex align-items-center">
                    <div class="flex-grow-1">
                        <li class="bx bx-edit"></li>&nbsp;&nbsp;
                        <span class="card-title" style="font-size: 15px;"> Form Product</span>
                    </div>
            ...
            <div class="card-body collapse show" id="collapseexample1">
                <form method="post" action="{{ isset($one) ? route('product.update', ['id' => $one->id]) : route('product.store') }}">@csrf
                    <fieldset>
                        <div class="row">
                            <div class="col-lg-4">
                                <label for="barcode" class="form-label mt-2">BARCODE</label>
                                <input name="barcode" type="text" class="form-control" value="{{ $one->barcode ?? old('barcode') }}">
                                <label class="form-label mt-2">NAMA</label>
                                <input name="name" type="text" class="form-control" value="{{ $one->name ?? '' }}">
                                <label for="choices-single-default" class="form-label mt-2">KATEGORI</label>
                                <select class="form-control js-example-basic-single" id="category_id" name="category_id">
                                    <option value="">Select</option>
                                    @if(isset($ms_category))
                                    @foreach($ms_category as $k => $v)
                                    <option value="{{ $v->id }}" <?= isset($one) && $one->category_id == $v->id ? 'selected=selected' : ''; ?>>{{ $v->name }}</option>
                                    @endforeach
                                    @endif
                                </select>
                            ...
                            <div class="col-lg-4">
                                @if(isset($ms_marketplace))
                                @foreach($ms_marketplace as $k => $v)
                                <label class="form-label mt-2">HARGA {{ $v }}</label>
                                <input name="price_{{  $v }}" type="number" class="form-control" value="{{ $mp_prices[$v] ?? '' }}">
                                @endforeach
                                @endif
                            </div>
                            <div class="col-lg-4">
                                @if(isset($ms_marketplace))
                                @foreach($ms_marketplace as $k => $v)
                                <label class="form-label mt-2">(%) POTONGAN {{ $v }}</label>
                                <input name="potongan_{{ $v }}" type="number" max="50" class="form-control" value="{{ $admin_fees[$v] ?? '' }}">
                                @endforeach
                                @endif
                            </div>
                        </div>
                            ...
                            <div class="col-lg-12">
                                <center>
                                    <div class="btn-group">
                                        ...
                                        <a class="btn btn-sm btn-danger" href="{{ route('product') }}">
            ...
            <div class="card-header pt-2 pb-2">
                <div class="d-flex align-items-center">
                    <div class="flex-grow-1">
                        ...
                        <span class="card-title" style="font-size: 15px;"> Data Product</span>
                    </div>
                    <div class="flex-shrink-0">
                    </div>
                </div>
                            ...
                            <td style="text-align: center;">
                                <div class="dropdown d-inline-block">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        ...
                                        <li><a href="{{ route('product.edit', ['id'=> $v->id]) }}" class="dropdown-item edit-item-btn">
...
<div class="modal fade" id="modalDelete" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <form action="{{ route('product.delete') }}" method="post">@csrf
